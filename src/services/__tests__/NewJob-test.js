...
describe('get job masters by jobMaster Ids', () => {

    it('should return jobMasterList', () => {
        let jobMasters = [
            {
                id: 123,
                title: 'Temp'
        ...
        expect(newJob.getMastersFromMasterIds(jobMasters, jobMasterIds)).toEqual(jobMasters)
    })

    it('should throw jobMasters missing error', () => {
        const message = 'configuration issues with job master'
        try {
            newJob.getMastersFromMasterIds(null, [{ id: 1 }])
        } catch (error) {
            expect(error.message).toEqual(message)
        }
    })

    it('should throw JobMasterIds missing error', () => {
        const message = 'configuration issues with next status in job master'
        try {
            newJob.getMastersFromMasterIds([{ id: 1 }],null)
        } catch (error) {
            expect(error.message).toEqual(message)
        }
    })
