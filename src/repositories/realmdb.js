...
export function getRecordListOnQuery(tableName, query, isSorted, sortProperty, reverse) {
    let records
    if (query) {
    ...
    if (isSorted && sortProperty) {
        let isReverse = (reverse) ? true : false
        records = records.sorted(`${sortProperty}`, isReverse)
    }
